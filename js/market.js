...
function roomOffice(){
  if(sessionsLeft<=0){
    ...
    $('glossBtn').addEventListener('click',roomGlossary);
  ...
  $('sheet').innerHTML=`<div class="roomhd"><h2>ARDENT CAPITAL</h2>
    ...
    <div class="cards">${['a','b'].map(k=>{const c=s[k];return `
      <button class="co" data-k="${k}" aria-pressed="false">
        ...
        ${row(termChip('growth','Revenue growth'),c.g+'%')}
    ...
    <div class="step"><div class="steplbl"><span>Instrument</span>
        <em>${unlockedInstruments().length} of ${INSTRUMENTS.length} desks open · ${termChip('xp',xp+' XP')}</em></div>
    ...
    <div class="step"><div class="steplbl"><span>${termChip('sizing','Position size')}</span><em>${arc===2?'Fund '+money(aum):'Portfolio '+money(port)}</em></div>
...
function sigHTML(){
  ...
  return `<section class="sig"><div class="steplbl"><span>${termChip('quadrant','Decision quality')}</span><em>Process scored separately from money</em></div>
...
function commit(){
  ...
  $('rev').innerHTML=`<div class="reveal"><div class="rvtop">
      ...
      <div class="chip"><span class="k">${termChip('process','Process')}</span>
    ...
    ${teachOnce('process','outcome')}${sound?'':teachOnce('sound')}
    ${tutAfter()}
